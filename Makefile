...
# http://github.com/jupyter/docker-stacks
	@echo "illumidesk/ga-stacks"
	@echo "====================="
	@echo "Replace % with a stack directory name (e.g., make build/ga-notebook)"
	@echo
...
build/%: ## build the latest image for a stack
	@docker build $(DARGS) --platform linux/amd64 --rm --force-rm -t $(OWNER)/$(notdir $@):$(TAG) ./$(notdir $@)
